...
for i = 1:N
    ...
    pop(i).Cost = CCT_Objectives_Non_rework(pop(i).Position);
end
archive = [];
...
for gen = 1:MaxGen
    ...
    for i = 1:2:N
        ...
        new_pop(i).Cost = CCT_Objectives_Non_rework(child1);
        new_pop(i+1).Position = child2;
        new_pop(i+1).Cost = CCT_Objectives_Non_rework(child2);
    end
    pop = new_pop;
...
xlabel('Economic Objective (f1)');
ylabel('Environmental Objective (f2)');
title('Final Pareto Front - SPEA2 for CCT Non-Rework');
grid on;
