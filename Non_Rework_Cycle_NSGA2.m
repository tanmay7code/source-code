...
% ===================== Evaluate parent population using CCT_Objectives_Non_rework =====================
fp1=zeros(pop,1); fp2=zeros(pop,1);
for p=1:pop
    xvec = xp(p,1:nv);
    fvals = CCT_Objectives_Non_rework(xvec);
    fp1(p,1)=fvals(1);
    fp2(p,1)=fvals(2);
end
...
for gen=1:maxgen
    ...
    % -------------------------
    % Evaluate child population using CCT_Objectives_Non_rework
    % -------------------------
